...
import { LayoutDashboard, ClipboardCheck, LogOut, ShieldCheck, Users, CreditCard, ReceiptText, Zap, Monitor, BadgeCheck, Megaphone, TrendingUp, Menu, X } from 'lucide-react';

const NAV_ITEMS = [
  { href: '/admin', label: '대시보드', icon: LayoutDashboard, exact: true },
  { href: '/admin/register-audit', label: '업소 입점 심사', icon: ClipboardCheck, exact: false },
  { href: '/admin/business-verify', label: '사업자 인증 심사', icon: BadgeCheck, exact: false },
  ...
  { href: '/admin/mrr', label: 'MRR 분석', icon: TrendingUp, exact: false },
  ...
  { href: '/admin/notices', label: '공지사항 관리', icon: Megaphone, exact: false },
] as const;
...
export default function AdminSidebar({ email }: AdminSidebarProps) {
  ...
  return (
    <>
      {/* 모바일 햄버거 버튼 */}
      <button
        className="fixed top-4 left-4 z-[57] lg:hidden p-2 bg-zinc-900 border border-zinc-800 rounded-lg text-zinc-400 hover:text-white transition-colors"
      ...
      <aside className={`w-64 border-r border-zinc-900 flex flex-col fixed inset-y-0 shadow-2xl z-[56] bg-zinc-950/50 backdrop-blur-xl transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="p-6 lg:p-8">
          <Link href="/admin" className="flex items-center space-x-3 group" onClick={() => setSidebarOpen(false)}>
